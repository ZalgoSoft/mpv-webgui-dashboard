            ...
            try:
                ...
                for idx, prop in enumerate(mpv_data["properties"]):
                    ...
                    if info.get('error') == 'success' and info.get('data'):
                        ...
                        # If property not found, treat as String
                    ...
                    if value.get('error') == 'success':
                        ...
                        mpv_data["property_values"][prop] = None
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .filter-buttons { display: flex; gap: 5px; flex-wrap: wrap; margin-bottom: 12px; }
        .filter-btn { 
            padding: 3px 10px; 
            background: #1a1a1a; 
            border: 1px solid #2a2a2a; 
        ...
        .filter-btn:hover { border-color: #ff6b6b; color: #e8e8e8; }
        .filter-btn.active { background: #ff6b6b; color: #0d0d0d; border-color: #ff6b6b; }
        .grid { 
            display: grid; 
        ...
        .cmd-arg .arg-name { color: #ff6b6b; font-weight: 500; }
        .cmd-arg .arg-type { color: #888; font-size: 11px; }
        .cmd-arg .arg-optional { color: #666; font-size: 10px; }
        ...
        .cmd-arg input:focus, .cmd-arg select:focus { outline: none; border-color: #ff6b6b; }
    ...
    grid.innerHTML = filtered.map(c => {
        ...
        if (c.args && c.args.length) {
            argsHtml = c.args.map((a, idx) => {
                ...
                if (a.type === 'Flag') {
                    inputHtml = `<input type="checkbox" id="cmd_${c.name}_${idx}" style="width:16px;height:16px;accent-color:#ff6b6b;">`;
            ...
            argsHtml = '<div style="color:#666;font-size:11px;padding:4px 0;">No arguments</div>';
        }
    ...
    grid.innerHTML = filtered.map(p => {
        ...
        return `
            <div class="card" id="prop-${p}" 
                ...
                <div class="type">${type}${info.default_value !== undefined ? ' | def: '+formatValue(info.default_value) : ''}</div>
                ...
                <div class="auto-update">
                    ...
                    <span>Auto</span>
                    <button class="btn btn-sm" onclick="getProp('${p}')" style="margin-left:auto;">↻</button>
                </div>
            </div>
        `;
    }).join('');
